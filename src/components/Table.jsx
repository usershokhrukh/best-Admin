...
import EditCarsModal from "./EditCarsModel";
...
const Table = ({data, status}) => {
  ...
  const reRender = () => {
    axios.get(`http://localhost:3000${location}`).then((dataRender) => {
      setDataItems(dataRender?.data);
    });
  };
  const handleDelete = (id) => {
    if (
      window.confirm(
        `Are you actually want to delete ${location.replace("/", "")}?`,
      )
    ) {
      axios
        .delete(`http://localhost:3000${location}/${id}`)
        .then((dataDelete) => {
          toast.warn(`${location.replace("/", "")} deleted!`);
          reRender();
        });
    } else {
      toast.success(`${location.replace("/", "")} saved!`);
    }
  };
  ...
  const [dataL, setDataL] = useState(Number(data?.[data.length - 1].id) + 1);
  return (
    <table className="main__table">
      <div className="main__table-top">
        <button
          onClick={() => {
            handleProductId(0, "products", "add");
            setAddOpened(true);
          }}
      ...
      {location === "/products" ? (
        <>
          <thead className="main__table-head">
            <th className="main__table-hd">Index</th>
          ...
          <tbody className="main__table-body">
            {dataItems?.map(({id, image, name, price, category}, index) => (
              <tr className="main__table-r main__table-animate">
                <td className="main__table-d">{index + 1}</td>
                ...
                <td className="main__table-d">
                  <span>
                    ...
                    <svg
                      onClick={() => {
                        handleProductId(id, "products", "edit");
                        setEditOpened(true);
                      }}
                      className="main__table-svgs"
                      xmlns="http://www.w3.org/2000/svg"
          ...
          {ViewOpened ? (
            <ViewProductsModal status={productId} opened={setViewOpened} />
          ) : null}

          {editOpened ? (
            <EditProductsModal
              status={productId}
              opened={setEditOpened}
        ...
        <>
          ...
          {ViewOpened ? (
            <ViewUsersModal status={productId} opened={setViewOpened} />
          ) : null}
        ...
        <>
          ...
          <tbody className="main__table-body">
            {dataItems?.map(
              ({id, image, make, model, price, color, year}, index) => (
                <tr className="main__table-r main__table-animate">
                  <td className="main__table-d">{index + 1}</td>
                  <td className="main__table-d">
                    <img
          ...
          {ViewOpened ? (
            <ViewCarsModal status={productId} opened={setViewOpened} />
          ) : null}
          {editOpened ? (
            <EditCarsModal
              status={productId}
